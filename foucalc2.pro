PRO foucalc2,time,rate,foupath, $
            dseg=inpdseg, $
            normindiv=normindiv, $
            ...
            zhang_dt=zhang_dt, $
            back_corr=back_corr, back_f = f_b, back_psd=psd_b, back_err=err_b, $ 
...
FOR chan=0,nch-1 DO BEGIN
    ...
    ;;
    IF keyword_set(back_corr) THEN BEGIN
        psdcorr_back,length,dseg, $
                     cfreq,noinormpd,avgback=[avg_bkg[0],avg_bkg[chan]], $
                     avgrate=mean(rate(*,chan)),$
                     back_f = f_b, back_psd=psd_b[*,chan], $
                     back_err=err_b[*,chan]
        noinormppsd(*,chan)=noinormpd & noinormpd=0. & cfreq=0.
    ENDIF
