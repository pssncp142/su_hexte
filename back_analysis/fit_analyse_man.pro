...
FOR i=0, n_elements(a)-1 DO BEGIN
   FOR j=0, 11 DO BEGIN
      intxuld=data[2,12*i+j]*f(a[i]-1,data[3,12*i+j])*4e2/$
              (xuld[j]-data[0,12*i+j]/rate[j])/rate[j]
      
      print, data[0,12*i+j]/rate[j], data[2,12*i+j]/rate[j],$
             intxuld,(data[0,12*i+j]/rate[j]+intxuld),$
             (data[0,12*i+j]/rate[j]+intxuld)/xuld[j]-1,data[3,12*i+j]
   ENDFOR
ENDFOR
