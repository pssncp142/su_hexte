...
FUNCTION exist, id ,l
  spawn,'ls '+string(id)+'/'+strtrim(l+30,1)+'-'+strtrim(l,1)+'-2.all'+ $
...
FUNCTION err_exist, id, l, err
  ndx = where(err[*,0] eq id and $
              err[*,1] eq strtrim(l+30,1)+'-'+strtrim(l,1)+'-2')
  RETURN, ndx
END
...
FOREACH l, lon DO BEGIN
   ...
   FOREACH id, ids DO BEGIN
      ...
      IF exist(id, l) EQ 1 AND err_exist(id, l, err) EQ -1 THEN BEGIN
         print, id, l
         sync_28,string(id)+'/'+strtrim(l+30,1)+'-'+strtrim(l,1)+'-2.all', $
                 t_t, r_t
         r = [r, r_t]
         help,r
   ...
   xdrlc_w,'arch/'+strtrim(l+30,1)+'-'+strtrim(l,1)+'-2/light/processed/'+$
