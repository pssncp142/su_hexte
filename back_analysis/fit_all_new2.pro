...
FUNCTION fit, f, psd, err, rate, xuld, k=mn_k
  pi = replicate({fixed:0, limited:[0,0], limits:[0.D,0.D]},8)

  IF keyword_set(mn_k) THEN BEGIN
     print,mn_k
    ; stop
  ENDIF
  ...
  start=[1D, rate, xuld, 0.5D, 2.5e-3, 1.7D, mn_k, 30e-3]
...
rate = [40.58, 45.02, 50.67, 47.65, 47.23, 47.25, 46.75, 45.94, $
       41.51, 35.65, 39.55, 40.54]
;rate = [40.59, 44.61, 50.02, 46.96, 46.63, 46.89, 46.44, 45.57, $
;       40.90, 35.33, 39.37, 40.50]

k = [0.15,0.16,0.17,0.18,0.19,0.2,0.21,0.22,0.23,0.24,0.25]

f_path = '/light/fourier/high/onelength/'
fit_r = fltarr(n_elements(lon), n_elements(k), 8)
...
FOR i=0, n_elements(lon)-1 DO BEGIN
   FOR j=0, n_elements(k)-1 DO BEGIN
       ...
       fit_r[i, j, *] = fit(f, psd, err, rate[i], xuld[i], k=k[j])
       device,decomposed=0
       loadct,1
       tt = findgen(256)
...
openw, 1, 'fit_res_man_k_fixed.txt'
FOR i=0, n_elements(lon)-1 DO BEGIN
      FOR j=0, n_elements(k)-1 DO BEGIN
      printf, 1, fit_r[i, j, 1], fit_r[i, j, 2], fit_r[i, j, 3], $
              fit_r[i, j, 6], fit_r[i, j, 7] 
   ENDFOR
ENDFOR
